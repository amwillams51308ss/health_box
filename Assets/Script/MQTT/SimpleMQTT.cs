...
public class SimpleMQTTUnity : MonoBehaviour
{
    ...
    [SerializeField] private string subscribeTopic = "";
    ...
    public static SimpleMQTTUnity Instance { get; private set; }
    ...
    void Awake()
    {
        // 單例：如果已經有一個 Instance，而且不是自己，就把自己砍掉
        ...
        while (wantReconnect)
        {
            ...
            if (client == null || !client.IsConnected)
            {
                if (!isConnecting)
                {
                    ...
                    {
                        ShowConnectSuccess($"MQTT 已連線：{brokerIP}:{brokerPort}");
                        // 連線成功後，重新訂閱所有 Topic
    ...
    private void ShowConnectSuccess(string msg)
    {
        Debug.Log(msg);
